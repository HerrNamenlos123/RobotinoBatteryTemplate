...
bool BatteryApp::OnStartup() {		// Called once on startup, engine is initializedby  now
	...
	robotinoLayer = std::make_shared<RobotinoLayer>();	// Robotino layer is created and pushed onto the layer stack as a normal layer
	PushLayer(robotinoLayer);							// (PushLayer() or PushOverlay(), overlays are always above normal layers)
...
BatteryApp& BatteryApp::Get() {			// This lets you access this class from anywhere, DO NOT ALTER!
	return static_cast<BatteryApp&>(Battery::GetApp());
}
